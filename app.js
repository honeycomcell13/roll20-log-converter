...
function archiveMarkup(data,title,extraClass=""){return`<main class="archive${extraClass?` ${extraClass}`:""}">${coverData?`<div class="cover"><img src="${coverData}" alt="${esc(title)} 썸네일"></div>`:""}<header class="title"><h1>${esc(title)}</h1></header>${data.map((b,i)=>renderBlock(b,i,false)).join("")}</main>`}
function documentHtml(data,title){return`<!doctype html><html lang="ko"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(title)}</title><style>${exportCss}</style></head><body>${archiveMarkup(data,title)}</body></html>`}
function matchingBrace(css,open){let depth=0;for(let i=open;i<css.length;i++){if(css[i]==="{")depth++;if(css[i]==="}"&&--depth===0)return i}return css.length-1}
function scopedCss(css,scope){let out="",pos=0;while(pos<css.length){const open=css.indexOf("{",pos);if(open<0){out+=css.slice(pos);break}const head=css.slice(pos,open).trim(),close=matchingBrace(css,open);if(/^@media/i.test(head)){out+=`${head}{${scopedCss(css.slice(open+1,close),scope)}}`}else{const scoped=head.split(",").map(raw=>{const s=raw.trim();if(s==="*")return`${scope},${scope} *`;if(s==="body")return scope;if(s===".archive")return`${scope}.archive`;return`${scope} ${s}`}).join(",");out+=`${scoped}{${css.slice(open+1,close)}}`}pos=close+1}return out}
function tistoryHtml(data,title){return`<style>${scopedCss(exportCss,".roll20-tistory")}</style>${archiveMarkup(data,title,"roll20-tistory")}`}
...
async function finalTistoryHtml(){const data=structuredClone(blocks),title=$("#title-input").value.trim()||"롤20 세션 로그";return tistoryHtml(data,title)}
async function copyText(text){const clean=String(text).replace(/\u0000/g,"");const area=document.createElement("textarea");area.value=clean;area.setAttribute("readonly","");area.style.cssText="position:fixed;left:-9999px;top:0;width:1px;height:1px;opacity:0";document.body.append(area);area.focus();area.select();area.setSelectionRange(0,area.value.length);let ok=false;try{ok=document.execCommand("copy")}catch{}area.remove();if(ok)return true;try{await navigator.clipboard.write([new ClipboardItem({"text/plain":new Blob([clean],{type:"text/plain"})})]);return true}catch{try{await navigator.clipboard.writeText(clean);return true}catch{return false}}}
$("#preview-button").onclick=async()=>{const html=await finalHtml(),url=URL.createObjectURL(new Blob([html],{type:"text/html"}));$("#preview-frame").src=url;$("#preview-dialog").showModal()};$("#preview-close").onclick=()=>$("#preview-dialog").close();
$("#copy-button").onclick=async()=>{const button=$("#copy-button"),label=button.textContent;button.disabled=true;button.textContent="복사 중…";try{const html=await finalTistoryHtml(),ok=await copyText(html);toast(ok?"티스토리용 HTML 코드를 복사했어요.":"복사하지 못했어요. 브라우저의 클립보드 권한을 확인해주세요.")}finally{button.disabled=false;button.textContent=label}};
$("#download-button").onclick=async()=>{const html=await finalHtml(),title=$("#title-input").value.trim()||"롤20 세션 로그",a=document.createElement("a");a.href=URL.createObjectURL(new Blob([html],{type:"text/html;charset=utf-8"}));a.download=title.replace(/[\\/:*?"<>|]/g,"_")+".html";a.click();setTimeout(()=>URL.revokeObjectURL(a.href),3000);toast("HTML을 저장했어요.")};
