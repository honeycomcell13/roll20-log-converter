...
async function finalTistoryHtml(){const data=$("#embed-images").checked?await embedAvatars():structuredClone(blocks),title=$("#title-input").value.trim()||"롤20 세션 로그";return tistoryHtml(data,title)}
async function copyText(text){try{await navigator.clipboard.writeText(text);return true}catch{const area=document.createElement("textarea");area.value=text;area.style.cssText="position:fixed;left:-9999px;top:0";document.body.append(area);area.select();const ok=document.execCommand("copy");area.remove();return ok}}
