...
function parseMarkdown(md){
  ...
  while(i<lines.length){
    ...
    if(sp){
      ...
      out.push({
        type:"speech",
        speaker:unescapeMd(sp[1]),
        body:unescapeMd(body.trim()),
        avatar:image
      });
...
function sync(el){
  ...
  if(el.dataset.field==="bodyHtml"){
    b.bodyHtml=sanitize(el.innerHTML);
    return;
  }
...
function start(content,name=""){
  ...
  $("#title-input").value=
    ...
    name.replace(/\.(md|markdown|txt|html)$/i,"")||
    "롤20 세션 로그";
...
$("#rich-input").addEventListener("paste",e=>{
  ...
  if(html){
    e.preventDefault();
    pastedHtml=html;
    $("#rich-input").innerHTML=sanitize(html);
  }else{
    pastedHtml=text;
  }
});
...
$("#cover-input").onchange=e=>{
  ...
  r.onload=()=>{
    ...
    $("#cover-label").textContent=file.name;
  };
...
async function embedAvatars(){
  ...
  $("#image-status").textContent=
    ok===urls.length
      ?`인장 ${ok}개를 HTML에 저장했어요.`
      :`인장 ${ok}/${urls.length}개 저장. 나머지는 원본 링크로 유지돼요.`;
...
$("#reset-button").onclick=()=>{
  if(!confirm("현재 편집 내용을 닫을까요?"))return;
