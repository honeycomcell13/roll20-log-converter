...
function renderBlock(b,i,edit=true){
  let h="";
  if(b.type==="speech"){const body=b.bodyHtml?sanitize(b.bodyHtml):inline(b.body),prev=blocks[i-1],same=prev?.type==="speech"&&prev.speaker===b.speaker,nextTable=blocks[i+1]?.type==="table"&&blankSpeech(b),continuation=same?" continuation":"";if(gmSpeakers.has(b.speaker))h=`<div class="message gm-journal${continuation}"><span class="editable" ${edit?'contenteditable="true" data-field="bodyHtml"':""}>${body}</span></div>`;else{const profile=same?"":`${b.avatar?`<img class="avatar" src="${esc(b.avatar)}" alt="${esc(b.speaker)} 인장">`:`<span class="avatar placeholder">${esc(initials(b.speaker))}</span>`}<span class="by editable" ${edit?'contenteditable="true" data-field="speaker"':""}>${esc(b.speaker)}</span><span>:</span> `;h=`<div class="message general${continuation}${nextTable?" roll-speaker":""}"${styleAttr(speechStyle(b.sourceStyle))}>${profile}<span class="editable speech-body" ${edit?'contenteditable="true" data-field="bodyHtml"':""}>${body}</span></div>`}}
  if(b.type==="narration"){const body=b.bodyHtml?sanitize(b.bodyHtml):inline(b.body),hasOriginalStyle=!!b.sourceStyle||/\sstyle=["']/i.test(body),cl=hasOriginalStyle?"":narrationClass(textOf(body)),continuation=blocks[i-1]?.type==="narration"?" continuation":"";h=`<div class="message desc${continuation}"${styleAttr(b.sourceStyle)}><span class="editable ${cl}" ${edit?'contenteditable="true" data-field="bodyHtml"':""}>${body}</span></div>`}
