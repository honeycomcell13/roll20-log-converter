...
function htmlRollSpeech(el,lastSpeaker,lastAvatar){
  const currentSpeaker=el.querySelector(".by")?.textContent?.replace(/:\s*$/," ").trim()||"",currentAvatar=el.querySelector(".avatar img")?.src||"",speaker=currentSpeaker||lastSpeaker||"이름 없음",avatar=currentAvatar||lastAvatar||"";
...
function parseRoll20Html(html){
  ...
  [...d.body.children].forEach(el=>{
    ...
    if(el.matches(".message.rollresult")){const roll=htmlRollSpeech(el,lastSpeaker,lastAvatar);if(roll.explicitSpeaker)lastSpeaker=roll.speaker;if(roll.explicitAvatar)lastAvatar=roll.avatar;out.push(roll);return}
